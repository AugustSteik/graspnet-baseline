...
class GraspNetDataset(Dataset):
    def __init__(self, root, valid_obj_idxs, grasp_labels, camera='kinect', split='train', num_points=20000,
        ...
        for x in tqdm(self.sceneIds, desc = 'Loading data path and collision labels...'):
            for img_num in range(image_range[0], image_range[1]):
    ...
    def get_data_label(self, index):
        ...
        for i, obj_idx in enumerate(obj_idxs):
            if obj_idx not in self.valid_obj_idxs:  # NOTE: Skip any objects in the scene if they were not passed to hte class initially. 
                continue
