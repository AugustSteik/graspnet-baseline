...
def inference(net=gnet, dataset=TEST_DATASET, dataloader=TEST_DATALOADER, object_id = 9, hook_fn=None):
    net.eval()
    ...
    for batch_idx, batch_data in enumerate(dataloader):
        ...
        with torch.no_grad():
            try:
                logits, prediction = net(batch_data, point_cloud=batch_data['point_clouds'], object_id=9)
            except Exception:
    ...
    if hook_handle is not None:
        hook_handle.remove()
    return gg, end_points, grasps_per_object, best_grasp_per_object
...
if __name__=='__main__':
    grasp_group, end_points, grasps_per_object, best_grasp_per_object = inference()
