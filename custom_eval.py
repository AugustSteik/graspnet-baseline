...
class GraspNetEval(GraspNet):
    ...
    **Input:**
    ...
    def eval_scenes(self, scene_ids, dump_folder):
        '''
        **Input:**
    ...
    def eval_custom(self, scene_ids, dump_folder, proc=2):
        '''
        **Input:**
        ...
        **Output:**
        ...
        - ap: float of the AP for all split.
        '''
        res = np.array(self.eval_scenes(scene_ids=scene_ids, dump_folder=dump_folder))
        threshold = 1e-8
        # ap = np.mean(res[np.abs(res) > threshold])
        # print('\nEvaluation Result:\n----------\n{}, AP={}'.format(self.camera, ap))
        res_mask = [np.abs(res) > threshold]
        masked_res = res[res_mask]
        ap = [np.mean(masked_res), np.mean(masked_res[0:30]), np.mean(masked_res[30:60]), np.mean(masked_res[60:90])]
        print('\nEvaluation Result:\n----------\n{}, AP={}, AP Seen={}, AP Similar={}, AP Novel={}'.format(self.camera, ap[0], ap[1], ap[2], ap[3]))
        return res, ap
